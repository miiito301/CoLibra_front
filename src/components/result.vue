<template>
  <div v-if="books.length > 0">
    <div
      v-for="book in books"
      :key="book.id"
      class="border bg-white border-blue-800 p-4 rounded-lg my-6 relative shadow-md"
    >
      <!-- 本の情報を flex-row で表示 -->
        ...
        <div class="flex flex-col justify-between md:ml-6 flex-1">
          <div>
            ...
            <div class="flex flex-col gap-1 text-gray-700 mt-2">
              <p>
                <span class="font-semibold">Published：</span>{{ book.volumeInfo.publishedDate }}
              </p>
              <p>
                <span class="font-semibold">Page：</span>{{ book.volumeInfo.pageCount }} pages
              </p>
            </div>
          </div>
